...
return Cities = {
    data : {
        ...
        "0_0" : ["东城区","西城区","崇文区","宣武区","朝阳区","丰台区","石景山区","海淀区","门头沟区","房山区","通州区","顺义区","昌平区","大兴区","怀柔区","平谷区","密云县","延庆县"],
        "0_1" : ["和平区","河东区","河西区","南开区","河北区","红桥区","塘沽区","汉沽区","大港区","东丽区","西青区","津南区","北辰区","武清区","宝坻区","蓟县","宁河县","静海县"],
        "0_2" : ["黄浦区","卢湾区","徐汇区","长宁区","静安区","普陀区","闸北区","虹口区","杨浦区","闵行区","宝山区","嘉定区","浦东新区","金山区","松江区","青浦区","南汇区","奉贤区","崇明县"],
    ...
    change : function(idx){
        ...
        for(i=0; i<idx; i++){
            str += ("_" + ($(this.selIds[i])[0].selectedIndex-1));
        };
        var ss=$(this.selIds[idx])[0];
        with (ss) {
            length = 0;
            options[0] = new Option(this.selOpts[idx], this.selOpts[idx]);
            if(idx && $(this.selIds[idx-1])[0].selectedIndex>0 || !idx) {
                if(this.has(str)){
                    ar = this.data[str];
                    for(i=0;i<ar.length;i++){
                        var opt = new Option(ar[i], ar[i]);
                        if(ar[i] == this.initOpts[idx]) {
                            opt.selected = true;
                        }
                        options[length] = opt;
                    }//end for
                    // if(idx){ options[0].selected = true; }
                }
            }//end if idx

            if(++idx<this.selIds.length){
                this.change(idx);
            }
        }//End with
    },

    selIds : ["#s_prov", "#s_city"],
    selOpts : ["省份", "地级市"],
    initOpts : ["",""],

    init : function() {  //初始化函数
        $(this.selIds[0]).change(function() {
            Cities.change(1);
        });
        this.change(0);
    }
